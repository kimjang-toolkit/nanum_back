...
import { mapToAttendeeList } from 'mappers/mapToAttendeeList';

const ddbDocClient = createDynamoDBDocClient();

export const getAttendeeListDAO = async (ownerName: string, id: string): Promise<Attendee[]> => {
    const params = {
    ...
    try {
        ...
        if (result.Items && result.Items.length > 0) {
            ...
            const cobuying: Attendee[] = mapToAttendeeList(result.Items[0]);
